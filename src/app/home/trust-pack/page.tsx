...
export default function TrustPackPage() {
    ...
    const [trustFundable, setTrustFundable] = useState<boolean>(false);
    useEffect(() => {
        fetchTrustPlans();
        fetchTrustFunds();
    }, []);
    ...
    const fetchTrustFunds = async () => {
        try {
            ...
            const data = await response.json();
            if (data.success) {
                setAvailableFunds(data.data.totalAvailableFunds);
                setTrustFundable(data.data.trustFundable);
            }
        } catch (error) {
                ...
                <Box sx={{ textAlign: 'center', mb: 6 }}>
                    ...
                    {!trustFundable ? (
                        ...
                        >
                            ...
                            >
                                You cannot lock funds in a trust pack at this time. This could be because your available balance is $0 or you have a pending withdrawal. Please ensure you have sufficient funds and no pending withdrawals before attempting to create a trust fund.
                            </Typography>
                        </MotionPaper>
                    ) : (
                        <>
