...
async function checkPendingTransactions() {
    try {
        ...
        for (const transaction of pendingTransactions) {
            if (!transaction.toAddress || !transaction.chain || !transaction.token) {
                continue;
            }
            const balance = await walletService.getBalance(transaction.toAddress, transaction.chain as 'Binance' | 'Ethereum' | 'Tron', transaction.token);
            if (balance > 0) {
                ...
                transaction.token = transaction.token || 'USDT';
                transaction.amount = balance;
                transaction.amountInUSD = balance * (cryptoPrice[0]?.price || 1);
                await deposit(transaction.fromUserId as string, transaction.amountInUSD);
                transaction.releaseDate = new Date();
                await transaction.save();

                await DepositWalletModel.updateOne({ address: transaction.toAddress, chain: transaction.chain, userId: transaction.fromUserId as string }, { $set: { deposited: true } });
            }
            // try {
